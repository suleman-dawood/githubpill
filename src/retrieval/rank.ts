...
/**
 * Dedupe hits by canonical URL, merge their provenance, and score them.
 *
...
export function rankCandidates(hits: readonly RawHit[], maxCandidates: number): Candidate[] {
  ...
  for (const hit of hits) {
    const key = canonicalUrl(hit.url) || `${hit.source}:${hit.id.toLowerCase()}`;
  ...
  const candidates = [...groups.values()].map((group): Candidate => {
    const score =
      3 * group.sources.size +
      2 * group.queries.size +
      group.rankBonus +
      Math.log10(group.stars + 1);
